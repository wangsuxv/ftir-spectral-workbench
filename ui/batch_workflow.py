...
PAGES = (
    "1. 普通光谱导入与检查", "2. 单位与处理范围", "3. 逐谱粗调",
    "4. 逐谱细调", "5. 批量检查与导出",
)
UNITS = ("absorbance", "percent_transmittance", "fraction_transmittance")
UNIT_LABELS = {"absorbance": "A（吸光度）", "percent_transmittance": "%T（百分比透过率）",
               "fraction_transmittance": "T（分数透过率）"}
COARSE_METHODS = ("arpls", "asls", "airpls", "pspline_arpls", "rubberband", "offset", "linear", "none")
...
def spectrum_table(ws: BatchWorkspace) -> pd.DataFrame:
    ...
    for sid in ws.display_order:
        ...
        rows.append({
            "光谱 ID": sid, "展示名称": r.display_name, "源文件": source.original_filename,
            "原始列名": r.original_column_label, "原始列序号（x=0）": r.original_column_index,
            "单位": UNIT_LABELS[r.confirmed_input_unit], "最小波数": float(r.wavenumber.min()),
            "最大波数": float(r.wavenumber.max()), "点数": r.wavenumber.size,
            "方向": r.original_axis_direction, "状态": status_label(ws, sid),
            "重复候选": r.duplicate_candidate, "编码": source.import_probe["selected_encoding"],
...
def _values(ws: BatchWorkspace, sid: str, stage: str) -> dict[str, Any]:
    ...
    if name not in state.display_preferences:
        if stage == "preparation":
            ...
            value = {"input_unit": p.input_unit, "high": high, "low": low,
...
def _sync(ws: BatchWorkspace, sid: str, stage: str) -> None:
    ...
    try:
        if stage == "preparation":
            p = PreparationConfig(
                input_unit=values["input_unit"], wavenumber_range=(values["high"], values["low"]),
                baseline_smoothing={"enabled": values["smoothing"], "estimate_only": True,
                                    "window_length": values["window_length"], "polyorder": values["polyorder"]},
...
def _widget(ws: BatchWorkspace, sid: str, stage: str, field: str,
            label: str, kind: str, **kwargs: Any) -> Any:
    ...
    return getattr(st, kind)(label, key=key, on_change=_changed,
                            args=(ws, sid, stage, field, key), **kwargs)
...
def _sidebar_selection(ws: BatchWorkspace) -> str | None:
    ...
    status = st.sidebar.selectbox("状态筛选", ("全部", "待处理", "粗调完成", "细调完成", "细调过期", "粗调过期", "计算失败", "已排除", "已跳过细调"))
    visible = [sid for sid in ws.display_order if (
        query.casefold() in (ws.records[sid].display_name + " " + ws.sources[ws.records[sid].source_id].original_filename).casefold()
        and (status == "全部" or status_label(ws, sid) == status)
    )]
    labels = {sid: f"{ws.records[sid].display_name} · {ws.sources[ws.records[sid].source_id].original_filename} · 列{ws.records[sid].original_column_index} · {sid[:6]}" for sid in ws.display_order}
    ...
    chosen = st.sidebar.selectbox("当前光谱", visible, index=index, format_func=label,
                                  key=f"batch_current_{ws.workspace_id}_{query}_{status}")
    ...
    selected = st.sidebar.multiselect("批量勾选（独立于当前光谱）", ws.display_order,
                                     format_func=label, key=targets_key)
...
def _import_page(ws: BatchWorkspace) -> None:
    ...
    with st.expander("文本解析选项"):
        ...
        encoding = st.selectbox("编码", ("auto", "utf-8", "utf-16", "utf-16-le", "utf-16-be", "gb18030", "cp1252"))
    ...
    uploads = st.file_uploader("上传普通光谱文本文件", type=["csv", "tsv", "tab", "txt", "dpt", "asc", "dat", "xy"], accept_multiple_files=True, key="batch_raw_uploads")
    units = []
    for index, upload in enumerate(uploads or []):
        units.append(st.selectbox(f"文件单位 · {upload.name} [{index + 1}]", UNITS,
                                  index=UNITS.index(unit), format_func=UNIT_LABELS.get,
                                  key=f"batch_upload_unit_{upload.file_id}"))
    if st.button("确认单位并导入", disabled=not uploads, type="primary"):
        try:
            options = TextImportOptions(delimiter=delimiter, decimal_mark=decimal,
                                        encoding=encoding, header_mode=header, skip_rows=int(skip),
                                        trim_empty_edge_columns=trim)
            added = []
            for upload, source_unit in zip(uploads, units, strict=True):
                added.extend(import_sources(ws, [(upload.name, upload.getvalue())],
                                            input_unit=source_unit, options=options))
...
def _record_header(ws: BatchWorkspace, sid: str) -> None:
    ...
    st.caption(f"{ws.sources[record.source_id].original_filename} · 第 {record.original_column_index} 强度列 · {UNIT_LABELS[record.confirmed_input_unit]} · {status_label(ws, sid)}")
    name_key = _key(ws, sid, "display", "name")
    name = st.text_input("展示名称", value=record.display_name, key=name_key)
    excluded = st.checkbox("从批量导出中排除此条目", value=record.excluded,
                           key=_key(ws, sid, "display", "excluded"))
...
def _preparation_page(ws: BatchWorkspace, sid: str) -> None:
    stage = "preparation"
    _widget(ws, sid, stage, "input_unit", "此谱强度单位", "selectbox", options=UNITS, format_func=UNIT_LABELS.get)
    ...
    _widget(ws, sid, stage, "window_length", "估计平滑窗口点数（奇数）", "number_input", min_value=3, step=2)
    ...
    _widget(ws, sid, stage, "floor", "透过率 floor（输入单位）", "number_input", min_value=1e-15, format="%.8g")
    error = ws.states[sid].display_preferences.get("error_" + stage)
    if error:
        st.error(error)
    if st.button("确认此谱单位与处理范围", disabled=bool(error), type="primary") and _attempt(confirm_preparation, ws, sid):
...
def _coarse_editor(ws: BatchWorkspace, sid: str) -> None:
    ...
    _widget(ws, sid, "coarse", "p", "AsLS p", "number_input", min_value=1e-8, max_value=0.49999, format="%.6g")
...
def _fine_editor(ws: BatchWorkspace, sid: str) -> None:
    ...
    _widget(ws, sid, "fine", "endpoint_window_width_cm1", "端点窗口宽度 cm⁻¹", "number_input", min_value=0.0001)
    ...
    _widget(ws, sid, "fine", "polynomial_order", "细调多项式阶数", "number_input", min_value=1, max_value=3)
    if method in {"piecewise_linear", "pchip", "polynomial"}:
        if st.button("预填覆盖当前范围的锚点"):
            ...
            _values(ws, sid, "fine")["anchors"] = [dict(start=float(c-width), end=float(c+width), enabled=True, statistic="median") for c in centers]
            _sync(ws, sid, "fine")
            st.rerun()
        table = pd.DataFrame(_values(ws, sid, "fine")["anchors"], columns=["enabled", "start", "end", "statistic"])
        edited = st.data_editor(table, num_rows="dynamic", hide_index=True, key=_key(ws, sid, "fine", "anchors"),
                                column_config={"enabled": st.column_config.CheckboxColumn(default=True),
                                               ...
                                               "statistic": st.column_config.SelectboxColumn(options=["median", "mean"], default="median")})
        _values(ws, sid, "fine")["anchors"] = edited.astype(object).where(edited.notna(), None).to_dict("records")
        _sync(ws, sid, "fine")
...
def _plot_snapshot(ws: BatchWorkspace, sid: str, stage: str) -> None:
    state = ws.states[sid]
    choice = st.radio("显示结果", ("当前预览", "已确认结果"), horizontal=True,
    ...
    if choice == "已确认结果":
        ...
        try:
            ...
            config = coarse_config(state.preparation_committed, state.coarse_draft) if parent is None else fine_config(parent.config, state.fine_draft)
            if snapshot.fingerprint != stage_fingerprint(ws, sid, config, stage, parent.fingerprint if parent else None):
    ...
    st.caption(f"{choice} · {snapshot.config.coarse_baseline.method} / {snapshot.config.fine_baseline.method} · 范围 {snapshot.config.wavenumber_range} cm⁻¹")
    if stage == "coarse":
        st.plotly_chart(coarse_preview_figure(result, 0), width="stretch")
    else:
        fig = fine_residual_figure(result, 0)
        fig.add_trace(go.Scatter(x=result.absorbance_selected.wavenumber,
                                y=result.baseline_estimation_spectra[0] - result.baseline.coarse_baseline[0],
                                name="拟合用估计残差（估计通道 − 粗基线）", line={"dash": "dash"}))
        st.plotly_chart(fig, width="stretch")
    ...
    unit = st.selectbox("校正谱派生显示单位", UNITS, format_func=UNIT_LABELS.get,
    ...
    try:
        ...
        return
    figure = go.Figure(go.Scatter(x=result.absorbance_selected.wavenumber, y=derived.values[0], name=UNIT_LABELS[unit]))
    ...
    st.dataframe(pd.DataFrame([{k: v[0] if v and v[0] is not None else "N/A" for k, v in metrics.items() if k not in skip}]), hide_index=True)
    st.caption("时间连续性、相邻谱变化和扰动趋势：不适用。无峰区指标显示 N/A，诊断警告不自动删除或裁负值。")
...
def _processing_page(ws: BatchWorkspace, sid: str, stage: str) -> None:
    ...
    disabled = bool(error) or (stage == "fine" and (not state.fine_draft.enabled or state.fine_draft.method == "none"))
    left, center, right = st.columns(3)
    if left.button(f"计算{title}预览", disabled=disabled, type="primary") and _attempt(preview, ws, sid):
    ...
    with st.expander("复制参数与批量预览"):
        ...
        if st.button(f"复制{title}参数到选中项", disabled=not targets or bool(error)):
            outcomes = copy_parameters(ws, sid, targets, stage=stage, include_range=copy_range, include_smoothing=copy_smooth)
        ...
        if st.button(f"为选中项计算各自{title}预览", disabled=not targets):
            with st.spinner("按各条自己的配方逐谱计算预览…"):
                errors = {target: ws.states[target].display_preferences.get("error_" + stage)
                          for target in targets}
                ready = [target for target in targets if not errors[target]]
                outcomes = preview_selected(ws, ready, stage=stage)
                outcomes.update({target: "草稿无效，未运行：" + error for target, error in errors.items() if error})
                st.session_state["batch_operation_report"] = outcomes
        if st.session_state.get("batch_operation_report"):
            st.dataframe(pd.DataFrame([{"光谱": ws.records[item].display_name, "结果": error or "成功（未自动确认）"} for item, error in st.session_state["batch_operation_report"].items() if item in ws.records]), hide_index=True)
    _plot_snapshot(ws, sid, stage)
    _export_controls(ws, sid, stage)


def _workspace_restore(ws: BatchWorkspace) -> None:
    """Workspace bundle upload is connected in the export phase."""


def _export_controls(ws: BatchWorkspace, sid: str | None, stage: str) -> None:
    """Stage artifact controls are connected in the export phase."""


def _workspace_save(ws: BatchWorkspace) -> None:
    """Workspace serialization is connected in the export phase."""
