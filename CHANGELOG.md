...
本文件记录 FTIR Spectral Workbench 的用户可见变化。当前 `0.2.0` 条目描述已通过本地验收并上传公开功能分支的发布候选；它尚未合并默认分支、打 tag 或创建正式 Release。
...
- 已上传公开功能分支和 Draft PR；未加入原始数据，也尚未合并默认分支、创建 release 或 tag。
