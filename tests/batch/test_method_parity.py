...
def synthetic_workspace(unit: str = "absorbance") -> tuple[BatchWorkspace, str]:
    ...
    a = .2 + .0001 * (x - 900) + .5 * np.exp(-((x - 1450) / 35)**2)
    a += .002 * np.sin(x / 7)
...
def assert_exact_decomposition(actual, expected) -> None:  # type: ignore[no-untyped-def]
    for name in ("coarse_baseline", "fine_baseline", "total_baseline", "corrected"):
        np.testing.assert_array_equal(getattr(actual.baseline, name), getattr(expected.baseline, name))
    np.testing.assert_array_equal(actual.absorbance_selected.wavenumber, expected.absorbance_selected.wavenumber)
    np.testing.assert_array_equal(actual.baseline_estimation_spectra, expected.baseline_estimation_spectra)
    np.testing.assert_array_equal(actual.analysis_data, expected.analysis_data)


@pytest.mark.parametrize("method", ["none", "offset", "linear", "arpls", "asls", "airpls", "rubberband", "pspline_arpls"])
...
@pytest.mark.parametrize("method,order", [("none", 1), ("endpoint_window_linear", 1), ("piecewise_linear", 1), ("pchip", 1), ("polynomial", 1), ("polynomial", 2), ("polynomial", 3)])
def test_all_fine_methods_equal_single_authoritative_run(method: str, order: int) -> None:
    ...
    set_fine_draft(ws, sid, FineBaselineConfig(method=method, polynomial_order=order, anchors=anchors))
    ...
    np.testing.assert_array_equal(fine.result.baseline.coarse_baseline, ws.states[sid].coarse_snapshot.result.baseline.coarse_baseline)
