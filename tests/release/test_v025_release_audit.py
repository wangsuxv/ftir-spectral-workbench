...
def test_v025_release_audit_passes_without_private_path_or_raw_data_output() -> None:
    summary = run_release_audit(REPOSITORY, expected_version="0.3.1")
    ...
    assert release_metadata["distribution_version"] == "0.3.1"
    assert release_metadata["workbench_version"] == "0.3.1"
...
def test_default_v025_metadata_check_rejects_v031_without_explicit_target() -> None:
    historical = audit_release_metadata(REPOSITORY)
    assert historical["status"] == "fail"
    assert historical["expected_version"] == "0.2.5"
    assert historical["distribution_version"] == "0.3.1"
    assert historical["workbench_version"] == "0.3.1"
    assert historical["failed_checks"] == [
        "distribution_version_is_0_2_5", "workbench_version_is_0_2_5",
    ]

    current = audit_release_metadata(REPOSITORY, expected_version="0.3.1")
    assert current["status"] == "pass"
    assert current["expected_version"] == "0.3.1"
    assert current["checks"]["distribution_version_is_0_3_1"] is True
    assert current["checks"]["workbench_version_is_0_3_1"] is True
...
def test_cli_expected_version_is_explicit_and_defaults_to_v025() -> None:
    ...
    assert parser.parse_args(["--expected-version", "0.3.1"]).expected_version == "0.3.1"
