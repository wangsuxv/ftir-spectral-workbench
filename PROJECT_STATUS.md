...
FTIR Spectral Workbench 的 v0.2.0 基线冻结版已在本地分支 `feat/v0.2-baseline-frozen` 完成发布验收并形成阶段提交。项目版本 metadata 为 `0.2.0`；同名功能分支已上传至公开 GitHub 仓库并通过 Draft PR 进入审查，尚未合并默认 `main`、打 tag 或创建 GitHub release。
...
可公开复核的完整结果保存在 `artifacts/validation/final/`。这些结果说明候选通过本地验收并已上传公开功能分支，但不表示已经合并默认分支或创建正式 Release。
